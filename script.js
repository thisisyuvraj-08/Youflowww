...
// --- Accountability Partner & Sleep Detection ---
let accountabilityActive = false;
let sleepDetectionActive = false;
let faceNotDetectedSince = null;
let eyesClosedSince = null;
let timerPausedByAccountability = false;
let timerPausedBySleep = false;
const ACCOUNTABILITY_TIMEOUT = 15000; // 15 seconds
const SLEEP_TIMEOUT = 10000; // 10 seconds
const faceStatusPrompt = document.getElementById('face-detection-status');
...
function showFaceStatus(message, danger = false) {
  faceStatusPrompt.textContent = message;
  faceStatusPrompt.classList.add('visible');
  faceStatusPrompt.style.backgroundColor = danger ? '#f44336' : '#6c63ff';
}
function hideFaceStatus() {
  faceStatusPrompt.classList.remove('visible');
}

function pauseTimerBySystem(reason) {
  if (!isTimerRunning) return;
  pauseTimer();
  if (reason === 'accountability') timerPausedByAccountability = true;
  if (reason === 'sleep') timerPausedBySleep = true;
  pauseAlertSound.play();
}
function resumeTimerBySystem(reason) {
  if (isTimerRunning) return;
  if (reason === 'accountability' && timerPausedByAccountability) {
    resumeTimer();
    timerPausedByAccountability = false;
    resumeAlertSound.play();
  }
  if (reason === 'sleep' && timerPausedBySleep) {
    resumeTimer();
    timerPausedBySleep = false;
    resumeAlertSound.play();
  }
}
...
function detectFaceLoop(video) {
  setInterval(async () => {
    if (!accountabilityActive && !sleepDetectionActive) return;
    const result = await faceapi.detectSingleFace(video, new faceapi.TinyFaceDetectorOptions()).withFaceLandmarks();
    const now = Date.now();
    // Accountability Partner: Face detection
    if (accountabilityActive) {
      if (!result) {
        if (!faceNotDetectedSince) faceNotDetectedSince = now;
        if (now - faceNotDetectedSince > ACCOUNTABILITY_TIMEOUT) {
          pauseTimerBySystem('accountability');
          showFaceStatus('No face detected! Timer paused.', true);
        }
      } else {
        faceNotDetectedSince = null;
        hideFaceStatus();
        resumeTimerBySystem('accountability');
      }
    }
    // Sleep Detection: Eyes closed
    if (sleepDetectionActive && result) {
      const leftEye = result.landmarks.getLeftEye();
      const rightEye = result.landmarks.getRightEye();
      const eyeOpen = isEyeOpen(leftEye) && isEyeOpen(rightEye);
      if (!eyeOpen) {
        if (!eyesClosedSince) eyesClosedSince = now;
        if (now - eyesClosedSince > SLEEP_TIMEOUT) {
          pauseTimerBySystem('sleep');
          showFaceStatus('Wake up buddy! Eyes closed detected.', true);
          pauseAlertSound.play();
        }
      } else {
        eyesClosedSince = null;
        hideFaceStatus();
        resumeTimerBySystem('sleep');
      }
    }
  }, 500);
}
...
function isEyeOpen(eyeLandmarks) {
  // Simple eye aspect ratio (EAR) calculation
  ...
  return ear > 0.22; // threshold, adjust if needed
}
...
// --- Settings Toggles ---
document.getElementById('accountability-toggle').addEventListener('change', e => {
  accountabilityActive = e.target.checked;
  if (accountabilityActive || sleepDetectionActive) startFaceDetection();
  if (!accountabilityActive) hideFaceStatus();
});
document.getElementById('sleep-detection-toggle').addEventListener('change', e => {
  sleepDetectionActive = e.target.checked;
  if (accountabilityActive || sleepDetectionActive) startFaceDetection();
  if (!sleepDetectionActive) hideFaceStatus();
});
